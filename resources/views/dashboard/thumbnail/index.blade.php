...
<body>
            ...
            <div class="page-breadcrumb">
                <div class="row align-items-center">
                    <div class="col-6">
                        ...
                        <h1 class="mb-0 fw-bold">Thumbnail</h1> 
                    </div>
                    <div class="col-6">
                        <div class="text-end upgrade-btn">
                            <button class="btn btn-primary text-white" onClick="create()">Tambah Thumbnail</button>
                        </div>
                    </div>
                </div>
            </div>
            <div class="container-fluid">
                <div class="row">
                    <div class="col-12">
                        <div class="card">
                            <div class="card-body">
                                <div class="table-responsive" id="listThumbnail">
                                </div>
                            </div>
                        </div>
                    </div>
    ...
    <script>
        ...
        function read() {
            $.get("{{ route('readThumbnail') }}", {}, function(data, status) {
                $("#listThumbnail").html(data);
            });
        }

        function create() {
            $.get("{{ route('createThumbnail') }}", {}, function(data, status) {
                $("#title").html('Create Thumbnail')
                $("#formInput").html(data);
                $("#modalForm").modal('show');
            });
        }

        $(document).on("submit", "#formThumbnail", function(e) {
            e.preventDefault();
        ...
        function edit(id) {
            $.get("{{ url('thumbnail/edit') }}/" + id, {}, function(data, status) {
                $("#title").html('Edit Thumbnail')
                $("#formInput").html(data);
                $("#modalForm").modal('show');
            });
        }
